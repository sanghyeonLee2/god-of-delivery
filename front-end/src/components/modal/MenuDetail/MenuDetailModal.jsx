...
function MenuDetailModal({ modalType, modalData }) {
  const { form, query, mutate } = useMenuDetails(modalData);
  if (query.isLoading) {
    return <Loading />;
  }
  return (
    <>
      <ModalContentWrap $modalType={modalType}>
        ...
        <MenuDetailProlog
          imgUrl={query.menuData?.imgUrl}
          name={query.menuData?.name}
        ...
        <OrderPrice
          defaultPrice={query.menuData?.price}
          menuCategories={query.menuData?.MenuCategories}
          watch={form.watch}
        />
      </ModalContentWrap>
      {modalType === "메뉴상세" && (
        <ModalBtn
          isLoading={mutate.isCartPosting}
          text={"장바구니에 담기"}
