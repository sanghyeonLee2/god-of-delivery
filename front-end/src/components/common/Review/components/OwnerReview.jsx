...
function OwnerReview({ownerReview, isOwner}) {
    ...
    return (
        <OwnerReviewWrap onSubmit={handleSubmit((data) => updateReview({id: ownerReview.id, content: data.content}))}>
            <FlexOnly justify={"space-between"}>
                <Font>사장님</Font>
                <Font size={"small"} color={"gray"}>{omittedDate(ownerReview?.createdAt)}</Font>
            </FlexOnly>
